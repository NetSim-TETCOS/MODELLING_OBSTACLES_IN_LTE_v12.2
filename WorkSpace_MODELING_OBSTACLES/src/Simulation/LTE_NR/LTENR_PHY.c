...
int UE_count, gnb_id, ue_id;
double PL_dB;
typedef struct stru_pathloss_data
{
	int GNB_ID;
	int UE_ID;
	double PL_dB;
	_ptr_ele ele;
}pathloss_data, * ptrpathloss_data;
ptrpathloss_data pathloss_list;
#define PATHLOSS_DATA_ALLOC() (struct stru_pathloss_data*)list_alloc(sizeof(struct stru_pathloss_data),offsetof(struct stru_pathloss_data,ele))
#define PATHLOSS_DATA_NEXT(entity)	(LIST_NEXT(entity))
#define PATHLOSS_DATA_ADD(info,e)		(LIST_ADD_LAST(&(info),(e)))
#define  PATHLOSS_DATA_REMOVE(ls, mem) (LIST_FREE((void**)(ls),(mem)))
												  ...
												  ptrLTENR_ASSOCIATEDUEPHYINFO assocInfo, int CA_ID)
{
	...
	ptrpathloss_data data = pathloss_list;
	bool flag = false;
	while (data) {
		if (info->ueId == data->UE_ID && info->gnbId == data->GNB_ID)
		{
			info->uplink.rxPower_dbm = info->uplink.txPower_dbm - info->dTotalLoss - data->PL_dB;
			info->downlink.rxPower_dbm = info->downlink.txPower_dbm - info->dTotalLoss - data->PL_dB;
			flag = true;
		}

		data = PATHLOSS_DATA_NEXT(data);
	}
...
#pragma endregion
void fn_read_pathloss_file()
{
	char str[BUFSIZ];
	if (fileOpen == 0)
	{
		sprintf(str, "%s/%s", pszAppPath, "obstacle_loss.txt");
		fp = fopen(str, "r");
		if (!fp)
		{
			fnNetSimError("Unable to open obstacle loss file %s\n", str);
			perror(str);
			return;
		}
		char data[BUFSIZ];
		while (fgets(data, BUFSIZ, fp))
		{
			...
			if (*data == '#' ||
				*data == 0)
				continue;
			fileOpen++;
		 ptrpathloss_data list = PATHLOSS_DATA_ALLOC();
		 sscanf(data, "$ENB_ID = %d UE_ID = %d PL_dB = %lf'\n'", &gnb_id, &ue_id, &PL_dB);
		 list->GNB_ID = gnb_id;
		 list->UE_ID = ue_id;
		 list->PL_dB = PL_dB;
		 PATHLOSS_DATA_ADD(pathloss_list, list);
